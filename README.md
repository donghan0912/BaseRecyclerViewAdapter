...
一行代码设置不同状态ViewLayout（支持EmptyView、ErrorView、Loading等任意类型的布局，需要给这些view绑定数据或者添加点击事件的话，只需重载一个方法即可）  
一行代码设置加载更多、加载失败等不同类型（支持LoadMore、LoadMoreError等任意类型的布局，需要给这些view绑定数据或者添加点击事件的话，只需重载一个方法即可）   
